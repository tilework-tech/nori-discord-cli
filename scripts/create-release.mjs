#!/usr/bin/env node
// Cut a nori-discord-cli release by pushing a `discord-cli-v<version>` tag, which triggers
// .github/workflows/discord-cli-release.yml to publish to npm via OIDC Trusted Publishing.
//
...
git(["tag", "-a", tag, "-m", `nori-discord-cli release ${version}`]);
git(["push", "origin", tag]);

console.log(
  `Pushed ${tag}. The release workflow will publish nori-discord-cli@${version} to npm.`
);
